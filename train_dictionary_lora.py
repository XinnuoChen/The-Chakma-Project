...
def build_training_examples(rows):
    """
    Build a conversational prompt-completion dataset:
      prompt = [system, user]
      completion = [assistant]

    This is a strong fit for dictionary lookup fine-tuning.
    """
    examples = []
    ...
    for row in rows:
        for variant in range(PROMPT_VARIANTS_PER_ROW):
            prompt_messages = [
                ...
                {
                    "role": "user",
                    "content": build_user_prompt(row["headword"], row["pos"], variant),
                },
            ]
            completion_messages = [
...
def write_modelfile(output_dir: str, ollama_base: str) -> str:
    """
    Writes a Modelfile inside OUTPUT_DIR.
    Since the adapter dir is OUTPUT_DIR/adapter, the relative path is ./adapter.
    """
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA GPU not detected. Training Llama 3.1 8B LoRA without a GPU is usually impractical."
        )
    ...
    # Keep at least 1 eval row if the dataset is very small
    if len(dataset) < 50:
    ...
    # Optional: save a few sample rows for reference
